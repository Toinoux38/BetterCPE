...
class PlanningProvider extends ChangeNotifier {
  ...
  DateTime _currentMonday = date_utils.DateUtils.getMondayOfWeek(DateTime.now());
  ...
  /// Get selected date
  DateTime get selectedDate => _currentMonday.add(Duration(days: _selectedDayIndex));
  ...
  bool get isToday => date_utils.DateUtils.isSameDay(selectedDate, DateTime.now());
  
  /// Get week range string for current view
  String get weekRangeString => date_utils.DateUtils.getWeekRangeString(_currentMonday);
  ...
  Future<void> loadWeekPlanning() async {
    ...
    result.when(
      success: (events) {
        _weekPlanning = _planningService.groupEventsByDay(events, _currentMonday);
